...
\label{eq:VAorigin}
\eea
In this case the $\lambda$ is the $4d$ Goldstinos associated with the breaking of $\mathcal{N}=1$ supersymmetry. This action is still invariant under a non-linear transformation:
...
The supergravity equivalent to the local supersymmetry Volkov-Akulov model \eqref{eq:VAsusy} can be obtained by choosing Kähler- and superpotential:
\bea 
K &= X\bar{X}\,,\\
W &= W_0 + M^2 X\,,
\eea
...
\subsection{The $\mathbf{D3}$-Brane Action and Supergravity}
\label{sec:susyD3}
